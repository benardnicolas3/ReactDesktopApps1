(function () {
    ...
    var resourcesDir = '../ReactDesktopApps1.Resources/';
    var winFormsAssemblyInfoPath = '../ReactDesktopApps1.AppWinForms/Properties/AssemblyInfo.cs';
    var consoleDir = '../ReactDesktopApps1.AppConsole/bin/Release/';
    var consoleExeName = 'ReactDesktopApps1-console.exe';
    ...
    gulp.task('www-copy-resources-lib', () => {
        return gulp.src('../ReactDesktopApps1.Resources/bin/Release/ReactDesktopApps1.Resources.dll')
    ...
    gulp.task('www-copy-resources', () => {
        return gulp.src(['./wwwroot/**/*',
            '!./wwwroot/platform.*'])
            .pipe(gulp.dest('../ReactDesktopApps1.Resources/'));
    });
    ...
    gulp.task('www-msbuild-web', () => {
        return gulp.src('./ReactDesktopApps1.csproj')
            .pipe(msbuild({
    ...
    gulp.task('www-msbuild-resources', () => {
        return gulp.src('../ReactDesktopApps1.Resources/ReactDesktopApps1.Resources.csproj')
            .pipe(msbuild({
                toolsVersion: MSBUILD_TOOLS_VERSION,                
                targets: ['Clean', 'Rebuild'],
                properties: {
    ...
    gulp.task('www-msbuild-console', () => {
        return gulp.src('../ReactDesktopApps1.AppConsole/ReactDesktopApps1.AppConsole.csproj')
            .pipe(msbuild({
                toolsVersion: MSBUILD_TOOLS_VERSION,                
                targets: ['Clean', 'Rebuild'],
                properties: {
    ...
    gulp.task('www-msbuild-winforms', () => {
        return gulp.src('../ReactDesktopApps1.AppWinforms/ReactDesktopApps1.AppWinforms.csproj')
            .pipe(msbuild({
                toolsVersion: MSBUILD_TOOLS_VERSION,                
                targets: ['Clean', 'Rebuild'],
                properties: {
    ...
    gulp.task('www-nuget-pack-winforms', function (callback) {
        ...
        var excludes = globule.find([
            '../ReactDesktopApps1.AppWinForms/bin/x86/Release/*.pdb',
            '../ReactDesktopApps1.AppWinForms/bin/x86/Release/*.vshost.*'
        ]);
        var includes = [
        { src: '../ReactDesktopApps1.AppWinForms/bin/x86/Release/*.*', dest: '/lib/net45/' },
        { src: '../ReactDesktopApps1.AppWinForms/bin/x86/Release/locales/*.*', dest: '/lib/net45/locales/' }];
        return nugetpack({
            id: title,
            title: title,
                version: version,
